...
A 2.5D height map describing building/terrain height on a regular grid.
...
Per-beam matrix representation of the beam pattern.
...
- GAN / UNet preprocess both use height map as geometry input
- feature-map mode loads beam maps from `u0`
- radiomap preprocessing extracts building mask and valid mask from labeled radiomaps
